...
def validate_knn(model, loader, criterion):
    ...
    for d in tqdm(enumerate(loader)):
        ...
        if valn==1 and i==1:
            continue
        #['masks_prep', 'images_prep', 'masks_prep_bin']
        img=d[1]['images_prep']
        labels=d[1]['masks_prep']
        labels_seg=d[1]['masks_prep_bin']
...
def train_knn(
    ...
    for global_state['it'],d in tqdm(enumerate(train_loader, global_state['it'] + 1)):
        #['masks_prep', 'images_prep', 'masks_prep_bin']
        model.prepare_fit(d['images_prep'],d['masks_prep'],d['masks_prep_bin'])
...
def main():
    ...
    if args.model in ('knn',):
        ...
        trainer=train_knn
        model=GUESS()
    elif args.model in ('stn',):
        trainer=train_knn
        model=STN()
    elif args.model in ('cnn',):
        trainer = train_cnn
        #model = CNN(32)
...
#                          1.0 * time_val / n_val if n_val > 0 else 0.0))
            if args.model in ('cnn',):
                ...
                criterion = nn.MSELoss()
                it, best_loss, best_it, epoch_loss, epoch_iou, epoch_final_loss, time_total, time_val, n_val = trainer(train_loader, valid_loader, model, criterion, None, None, epoch, args.eval_every, args.print_every, args.save_every, global_state)
